export function modal(options) {
    ...
    function _createModal(options) {
        // todo change class modal__backdrop to your class and add it to popup.css
        if (options.popUp){
            let
                elemPopUp = document.createElement('div'),
                modalTemplate = '<div class="modal__backdrop" data-dismiss="modal"><div class="modal__content"><div class="modal__header"><div class="modal__title" data-modal="title">{{title}}</div>',
                modalHTML;

            elemPopUp.classList.add('pop-up');
            modalHTML = modalTemplate.replace('{{title}}', options.title || '');
            elemPopUp.innerHTML = modalHTML;
            ...
            if (options.src) modalHTML = modalHTML.replace("imgSrc", options.src)
            if (options.discount) modalHTML = modalHTML.replace("discount", options.discount)
            if (options.price) modalHTML = modalHTML.replace("price", options.price)
